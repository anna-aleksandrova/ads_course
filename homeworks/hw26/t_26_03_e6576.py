...
def Prim(graph: list[dict], n: int, p, q):
    distances = [INF for _ in range(n+1)]
    distances[p] = 0
    sources = [None for _ in range(n+1)]
    pq = PriorityQueue()
    ...
    while not pq.empty():
        v = pq.extractMinimum()
        for u in graph[v]:
            if u in pq and graph[v][u] < distances[u]:
                distances[u] = graph[v][u]
                pq.updatePriority(u, distances[u])
                sources[u] = v
    spanning_tree = [[] for _ in range(n+1)]
    for i in range(1, n+1):
        if sources[i] is not None:
            spanning_tree[i].append(sources[i])
            spanning_tree[sources[i]].append(i)
    if q in spanning_tree[p] or p in spanning_tree[q]:
        return "YES"
    else:
        return "NO"
